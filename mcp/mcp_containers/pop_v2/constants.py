...
POP_MONGODB_DATABASE = os.getenv(
    "POP_MONGODB_DATABASE", "new_pdf_chunks_and_metadata"
)
POP_MONGODB_COLLECTION = os.getenv(
    "POP_MONGODB_COLLECTION", "merged_chunks_metadata"
)

# Legacy aliases (e.g. seed_pop_test_chemical_compliance.py)
DB_NAME = POP_MONGODB_DATABASE
COLLECTION_POP = POP_MONGODB_COLLECTION

EMBEDDING_API_URL = os.getenv("EMBEDDING_API_URL", "http://100.100.108.44:6001/embed")
POP_SIMILARITY_THRESHOLD = float(os.getenv("POP_SIMILARITY_THRESHOLD", "0.8"))
POP_TOP_K = int(os.getenv("POP_TOP_K", "5"))
POP_VECTOR_INDEX_NAME = os.getenv("POP_VECTOR_INDEX_NAME", "vector_index")
POP_VECTOR_NUM_CANDIDATES = int(os.getenv("POP_VECTOR_NUM_CANDIDATES", "100"))
POP_VECTOR_SEARCH_LIMIT = int(os.getenv("POP_VECTOR_SEARCH_LIMIT", "25"))
METADATA_CACHE_PATH = os.getenv(
    "POP_METADATA_CACHE_PATH",
    str(Path(__file__).resolve().parent / "data" / "pop_metadata_cache.json"),
)
METADATA_REFRESH_SECONDS = int(os.getenv("POP_METADATA_REFRESH_SECONDS", str(6 * 3600)))
MCP_HOST = os.getenv("POP_MCP_HOST", "0.0.0.0")
MCP_PORT = int(os.getenv("POP_MCP_PORT", "9002"))
POP_API_PORT = int(os.getenv("POP_API_PORT", "9003"))
...
You are AgriSeva-AI. You are a specialized AI assistant made only to help farmers in India. Your only job is to give useful information and advice for farming in India.
...
CITATION_QA_TEMPLATE = """You are AgriSeva-AI. You are a specialized AI assistant made only to help farmers in India. Your only job is to give useful information and advice for farming in India.
**2. Strict Geographical Rule:**
You must only answer questions about farming in India. If a user asks about farming in America, Europe, Africa, or any other country, you must not answer. Your knowledge and advice are for Indian soil, Indian climate, and Indian farming methods only.
