...
export interface IQuestionRepository {
  ...
  getYearAnalytics(
    ...
    isAdmin?: boolean,
    session?: ClientSession,
 ): Promise<{yearData: GoldenDatasetEntry[]; totalEntriesByType: number; totalVerifiedByType: number; moderatorBreakdown?: { moderatorName: string, count: number, moderatorHours?: number, auditorHours?: number, gateKeeperHours?: number }[]; questionSourceBreakdown?: { whatsapp: number; agriseva: number }; questionsAnsweredWithin120Min?: { whatsapp: number; agriseva: number }; averageResponseTime?: { whatsapp: number; agriseva: number }; questionsAnsweredAfter120Min?: { whatsapp: number; agriseva: number }; questionStateBreakdown?: QuestionStateBreakdownBySource;paeMetrics?: { assigned: number; submitted: number; closed: number } }>;
