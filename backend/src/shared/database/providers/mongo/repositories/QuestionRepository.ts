...
export class QuestionRepository implements IQuestionRepository {
      ...
      pipeline.push({
        $addFields: {
          priorityOrder: {
            $switch: {
              branches: [
                // AGRISEVA_AI / WHATSAPP
                {
                  case: {
                    $and: [
                      { $eq: ['$priority', 'critical'] },
                      { $in: ['$source', ['AGRISEVA_AI', 'WHATSAPP']] },
                    ],
                  },
                  then: 1,
                },
  ...
  async getQuestionSourceBreakdown(
    ...
    customStartTime?: string,
    customEndTime?: string,
  ): Promise<{ whatsapp: number; agriseva: number }> {
    await this.init();
    ...
    const whatsapp =
      ...
      0;
    const agriseva =
      sourceBreakdown.find(s => s._id?.toLowerCase() === 'agriseva')?.count ??
      0;

    return { whatsapp, agriseva };
  }
  ...
  async getQuestionsAnsweredWithin120Minutes(
    ...
    const agriseva =
      result.find(s => s._id?.toLowerCase() === 'agriseva')?.count ?? 0;
  ...
  async getQuestionStateBreakdown(
    ...
    const buildBreakdown = (sourceName: 'whatsapp' | 'agriseva') => {
    ...
    return {
      ...
      agriseva: buildBreakdown('agriseva'),
    };
  }
  ...
  async getAverageResponseTime(
    ...
    const agriseva = result.find(r => r.source === 'agriseva')?.avgTime ?? 0;
  ...
  async getDailyAnalytics(
    ...
    let questionSourceBreakdown: { whatsapp: number; agriseva: number } = {
      whatsapp: 0,
      agriseva: 0,
    };
    let questionsAnsweredWithin120Min: { whatsapp: number; agriseva: number } = {
      whatsapp: 0,
      agriseva: 0,
    };
    let averageResponseTime: { whatsapp: number; agriseva: number } = {
      whatsapp: 0,
      agriseva: 0,
    };
    let questionsAnsweredAfter120Min: { whatsapp: number; agriseva: number } = {
      whatsapp: 0,
      agriseva: 0,
    };
  ...
  async findUnassignedQuestionsForRole(
    ...
    const filter: Record<string, unknown> = {
      ...
      source: { $in: ['AGRISEVA_AI', 'WHATSAPP'] },
  ...
  async getQueueQuestionSection(
    ...
    startTime?: Date,
    endTime?: Date,
    sources: string[] = ['AGRISEVA_AI', 'WHATSAPP'],
