...
export class ChatbotRepository implements IChatbotRepository {
  ...
  private async getSourceAdherenceStats(
    source: 'WHATSAPP' | 'AGRISEVA_AI' | 'MANUAL',
    userType = 'all',
    startTime?: string,
    endTime?: string,
    dbSource?: string,
  ...
  async getResponseAdherenceTable(
    ...
    try {
      ...
      const [whatsapp, agriseva, manual] = await Promise.all([
      ...
      const agrisevaDynamicWeather = agriseva.dynamicWeatherCount;

      const whatsappDynamicMarket = whatsapp.dynamicMarketCount;
      const agrisevaDynamicMarket = agriseva.dynamicMarketCount;

      const whatsappDynamicSchemes = whatsapp.dynamicSchemesCount;
      const agrisevaDynamicSchemes = agriseva.dynamicSchemesCount;

      const totalUserMessages = queryCounts[0]?.count ?? 0;
      const whatsappQueriesAsked = 0;
      const agrisevaQueriesAsked = totalUserMessages;
      ...
      const whatsappAdherencePct =
        whatsapp.totalResponded > 0
          ...
          : 0;
      const agrisevaAdherencePct =
        agriseva.totalResponded > 0
          ? Math.round(
              (agriseva.answeredWithin120Min / agriseva.totalResponded) *
                100 *
                100,
            ) / 100
          : 0;
      ...
      const agrisevaSlaBreachedCount = Math.max(
        0,
        agriseva.totalResponded - agriseva.answeredWithin120Min,
      );
      ...
      // const agrisevaNonGdbWithin120 = agriseva.nonGdbWithin120;
      ...
      return {
        ...
        agrisevaClosedCount: agriseva.closedCount,
        agrisevaPendingCount: agriseva.pendingCount,
        agrisevaNonAgriCount: agriseva.nonAgriCount,
        agrisevaDynamicCount: agriseva.dynamicCount,
        agrisevaDuplicateCount: agriseva.duplicateCount,
        agrisevaHoldCount: agriseva.holdCount,
        agrisevaPaeSubmitedCount: agriseva.paeSubmitedCount,
        agrisevaDynamicCLosedCount: agriseva.dynamicCLosedCount,
        agrisevaReroutedCount: agriseva.reroutedCount,
        agrisevaPassCount: agriseva.passCount,
        agrisevaDuplicateClosedCount: agriseva.duplicateClosedCount,
        ...
        agrisevaAverageResponseGBDMinutes: agriseva.averageResponseGDBMinutes,
        agrisevaAverageResponseNonGBDMinutes:
          agriseva.averageResponseNonGDBMinutes,

        whatsappAverageResponseMinutes:
          Math.round(whatsapp.averageResponseMinutes * 100) / 100,
        agrisevaAverageResponseMinutes:
          Math.round(agriseva.averageResponseMinutes * 100) / 100,
        manualAverageResponseMinutes:
          Math.round(manual.averageResponseMinutes * 100) / 100,
        whatsappAdherencePct,
        agrisevaAdherencePct,
        manualAdherencePct,
        ...
        answeredWithin120MinClosedagriseva:
          agriseva.answeredWithin120MinClosed,
        answeredWithin120MinPassagriseva: agriseva.answeredWithin120MinPass,
        answeredWithin120MinDynamicClosedagriseva:
          agriseva.answeredWithin120MinDynamicClosed,
        answeredWithin120MinDuplicateClosedagriseva:
          agriseva.answeredWithin120MinDuplicateClosed,
        ...
        agrisevadynamicWeatherDynamicCount:
          agriseva.dynamicWeatherDynamicCount,
        agrisevadynamicWeatherStaticDynamicCount:
        ...
        agrisevadynamicMarketDynamicCount: agriseva.dynamicMarketDynamicCount,
        agrisevadynamicMarketStaticDynamicCount:
        ...
        agrisevadynamicSchemesDynamicCount:
          agriseva.dynamicSchemesDynamicCount,
        agrisevadynamicSchemesStaticDynamicCount:
        ...
        agrisevaAnsweredAfter120Min: agriseva.answeredAfter120Min,
        ...
        agrisevaAnsweredAfter120MinClosed: agriseva.answeredAfter120MinClosed,
        agrisevaAnsweredAfter120MinPass: agriseva.answeredAfter120MinPass,
        agrisevaAnsweredAfter120MinDynamicClosed:
          agriseva.answeredAfter120MinDynamicClosed,
        agrisevaAnsweredAfter120MinDuplicateClosed:
          agriseva.answeredAfter120MinDuplicateClosed,
        ...
        // AgriSeva-AI TAT
        agrisevaTatMinutes: agriseva.tatMinutes,
        agrisevaAverageTimeToAuthorMinutes:
          agriseva.averageTimeToAuthorMinutes,
        agrisevaAverageReviewAcceptMinutes:
          agriseva.averageReviewAcceptMinutes,
        agrisevaAverageReviewModifyMinutes:
          agriseva.averageReviewModifyMinutes,
        agrisevaAverageReviewRejectReauthorMinutes:
          agriseva.averageReviewRejectReauthorMinutes,
        agrisevaAverageModeratingMinutes: agriseva.averageModeratingMinutes,
        agrisevaAverageGatekeepingMinutes: agriseva.averageGatekeepingMinutes,
        agrisevaAverageAuditingMinutes: agriseva.averageAuditingMinutes,
        agrisevaAverageReroutedCompletionMinutes:
          agriseva.averageReroutedCompletionMinutes,
        ...
        agrisevaAverageEndToEndQnaCompletionMinutes:
          agriseva.averageEndToEndQnaCompletionMinutes,
        ...
        agrisevaAverageEndToEndUniqueMinutes:
          agriseva.averageEndToEndUniqueMinutes,
        ...
        agrisevaAverageEndToEndDynamicMinutes:
          agriseva.averageEndToEndDynamicMinutes,
        ...
        agrisevaAverageEndToEndDuplicateMinutes:
          agriseva.averageEndToEndDuplicateMinutes,
        ...
        agrisevaPaeAssignedQuestions: agriseva.paeAssignedQuestions,
        ...
        agrisevaPaeContributionToGDB: agriseva.paeContributionToGDB,
        manualPaeContributionToGDB: manual.paeContributionToGDB,
        // PAE Contribution to GDB %
        whatsappPaeContributionToGDBPct: whatsapp.paeContributionToGDBPct,
        agrisevaPaeContributionToGDBPct: agriseva.paeContributionToGDBPct,
  ...
  //without unwind
  private buildUserTypeLookupStages(
    userType: string,
    keepUserDoc: boolean = false,
  ): any[] {
    if (userType === 'all') return [];
  ...
  async getQueryCategories(
    ...
    try {
      ...
      const source = _source === 'whatsapp' ? 'WHATSAPP' : 'AGRISEVA_AI';
  ...
  async getDistrictAnalyticsByState(
    ...
    try {
      ...
      if (startDate || endDate) {
        ...
        if (startDate) {
          feedbackMatch.createdAt.$gte = startDate;
        }

        if (endDate) {
          feedbackMatch.createdAt.$lte = endDate;
        }
      }
      ...
      if (startDate || endDate) {
        districtUsers = await this.messagesCollection
          .aggregate([
            {
              $match: messageMatch,
            },
            ...
            {
              $group: {
                _id: '$user.farmerProfile.district',
        ...
        districtUsers = await this.users
          .aggregate([
            {
              $match: {
                'farmerProfile.state': {
  ...
  async getQuestionFromDistrict(
    ...
    try {
      ...
      const safeLimit = Math.min(Math.max(Number(limit) || 10, 1), 100);
      const skip = (safePage - 1) * safeLimit;
      const sourceType = source === 'whatsapp' ? 'WHATSAPP' : 'AGRISEVA_AI';
  ...
  async getFarmerHeatMapAnalytics(
    ...
    try {
      ...
      const finalSource: QuestionSource =
        source === 'whatsapp' ? 'WHATSAPP' : 'AGRISEVA_AI';
  ...
  async getCoordinatorDuplicateQuestionHeatMap(
    ...
    try {
      ...
      const questionFilter: any = buildBaseQuestionMatch('AGRISEVA_AI');
  ...
  async getFeedbackUsers(
    ...
    try {
      await this.init(source);
      const userTypeLookupStages = this.buildUserTypeLookupStages(
        userType,
        true,
      );
  ...
  async getUserDetails(
    ...
    try {
      ...
      const merged: UserDetailEntry[] = allUsers
        .filter(u => {
          // If no date filter, keep everyone
          if (!startDate && !endDate) {
            return true;
          }

          // With a date filter, only keep users who asked questions
          return (countMap.get(String(u._id)) ?? 0) > 0;
        })
        .map(u => ({
          userId: String(u._id),
          name: u.name || u.username || 'Unknown',
          ...
          createdAt: u.createdAt,
          isVerified: u.isVerified ?? true,
          farmerProfile: u.farmerProfile ?? undefined,
        }));
  ...
  async getDailyAnalyticsForWhatsApp(start: Date, end: Date): Promise<any> {
    ...
    const analyticsMap = new Map(analytics.map(item => [item.period, item]));
  ...
  private async buildUserQuestionScope(userId?: string): Promise<any | null> {
    ...
    if (ObjectId.isValid(userId)) {
      ...
      if (messageIds.length > 0)
        userMatches.push({messageId: {$in: messageIds}});
  ...
  async getAllStatesQuestionsAndUsersData(
    ...
    try {
      ...
      if (startDate || endDate) {
        usersByState = await this.messagesCollection
          .aggregate([
            {
              $match: messageMatch,
            },
  ...
  async getUserMessageMetricDetails(
    ...
    try {
      ...
      const toMessageItem = (message: any) => ({
        _id:
          message.messageId || message._id?.toString?.() || String(message._id),
      ...
      if (metric === 'longestConversation') {
        ...
        return paginate(
          (longestConversation?.messages ?? []).map(toMessageItem),
        );
      }
      ...
      if (metric === 'questionsFromMessages') {
        ...
        const userQuestionFilter: any = buildBaseQuestionMatch('AGRISEVA_AI');
  ...
  async getUserProfile(
    ...
    try {
      ...
      const trendUserMessages =
        dashboardUserMessages.filter(isInsideTrendRange);
      ...
      const farmerDashboard = {
        ...
        messagingMetrics: {
          ...
          latestConversationDate: conversations[0]?.conversationDate || null,
