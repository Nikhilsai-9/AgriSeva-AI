...
export class QuestionService extends BaseService implements IQuestionService {
  ...
  async getCallSummary(query: string): Promise<any> {
    try {
      const extractResponse = await axios.post(
        `${aiConfig.agentSearchUrl}/extract`,
        { query },
        { timeout: 100000 },
      );
  ...
  async addQuestion(
    ...
    try {
      ...
      if (source === 'AGRISEVA_AI' || source === 'WHATSAPP') {
        priority = 'high';
      }
    ...
    try {
      if (source === 'AGRI_EXPERT') {
        ...
        if (isTimeBoundedQuestion) {
          ...
          // AGRISEVA_AI / WHATSAPP — GDB → embedding → LLM duplicate/non-agri pipeline
          try {
            const result = await this.runDuplicateCheckPipeline(
              baseQuestion,
              details,
  ...
  async getQuestionById(questionId: string): Promise<QuestionResponse> {
    try {
      return this._withTransaction(async (session: ClientSession) => {
        ...
        if (
          currentQuestion.source === 'AGRISEVA_AI' &&
          !aiInitialAnswer &&
          answers &&
          answers.length > 0
        ) {
  ...
  async autoAllocateExperts(
    ...
    // Single-allocation sources (time-bound AGRISEVA_AI/WHATSAPP and manual
  ...
  async allocateExperts(
    ...
    try {
      return this._withTransaction(async (session: ClientSession) => {
        ...
        // refresh for AGRISEVA_AI/WHATSAPP questions).
        if (question.source === 'WHATSAPP' || question.source === 'AGRISEVA_AI') {
  ...
  async sendOutReachQuestionsMail(
    ...
    try {
      ...
      // const duplicateQuestions = await this.duplicateQuestionRepository.findDuplicatesByDateRange(start, end, 'AGRISEVA_AI');
      ...
      //   emails,
      //   'AgriSeva-AI Outreach Questions Report',
      //   `
      //     <p>Hello,</p>
      //     <p>Please find attached the <b>AgriSeva-AI Outreach Questions</b> report.</p>
      //     <p>Date Range: <b>${startDate}</b> to <b>${endDate}</b></p>
      //     <br />
      //     <p>Regards,<br/>AgriSeva-AI System</p>
      //   `,
      //   csv,
...
if (filters.endDate) {
  ...
  async generateDuplicateQuestionReport(
    ...
    return this._withTransaction(async session => {
      ...
      // const duplicateQuestions = await this.duplicateQuestionRepository.findDuplicatesByDateRange(startDate, endDate, 'AGRISEVA_AI', session);
      const duplicateQuestions =
        await this.duplicateQuestionRepository.findDuplicatesByDateRange(
          startDate,
          endDate,
  ...
  async getMatchedQuestion(questionId: string) {
    ...
    const isTimeBoundedQuestion =
      questionSource === 'WHATSAPP' || questionSource === 'AGRISEVA_AI';
  ...
  /** Periodic job — handles three cases for time-bound (AGRISEVA_AI/WHATSAPP) questions:
