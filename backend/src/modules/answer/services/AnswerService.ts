    ...
    try {
      await this._withTransaction(async (session: ClientSession) => {
        ...
        if (currentUserIndexInQueue !== -1) {
          ...
          // Single-allocation questions (all sources: AGRISEVA_AI / WHATSAPP /
...
answer: ${updates.answer}`;
      ...
      if (question.status === 'closed' && updates.answerId) {
        ...
        // NOTE: Re-firing the WhatsApp / AgriSeva-AI webhook on edit is disabled
        ...
        // }
        //
        // if (question.source === 'AGRISEVA_AI') {
        //   try {
        //     await triggerWebhook(
        ...
        //     );
        //   } catch (err) {
        //     console.log('Error occurred while notifying customer on edit (AGRISEVA_AI):', err);
        //   }
        // }
   ...
   * source-appropriate webhook (WhatsApp for WHATSAPP, Browser for AGRISEVA_AI), and
  ...
  private async notifyCustomerOnClose(
    ...
    if (q.source !== 'WHATSAPP' && q.source !== 'AGRISEVA_AI') return false;
