 ...
 *   source in [AGRISEVA_AI, WHATSAPP]  AND  isAutoAllocate: true
 *
...
const TIME_BOUND_SOURCES = ['AGRISEVA_AI', 'WHATSAPP'];
...
try {
  ...
  console.log(
    `\n📅 Window : ${istLabel(from)} → ${istLabel(to)}` +
      `\n🎯 Scope  : ${ALL_SOURCES ? 'ALL sources' : 'time-bound (AGRISEVA_AI/WHATSAPP + isAutoAllocate)'}` +
