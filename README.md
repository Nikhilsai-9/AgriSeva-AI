...
AgriSeva-AI follows a smart **three-tier approach** to find the best answer:
...
- **AI models:** Claude (Anthropic), MiniMax (self-hosted), Gemma (self-hosted)
...
4. **Continuous review** — each AI-generated answer is simultaneously forwarded to the **AgriSeva-AI Reviewer System** for expert validation. As more answers are reviewed and approved, the Golden Dataset grows, so more farmers receive instantly verified answers over time.
...
See [DEPLOYMENT_SETUP.md](./DEPLOYMENT_SETUP.md) for complete environment configuration and deployment instructions.

Copy the root `.env.example` to `.env` and fill in all required values before starting.
