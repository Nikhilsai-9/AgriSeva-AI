...
async def _stream_agriseva_response(body: dict, request: Request):
...
@app.api_route("/v1/agriseva/models", methods=["GET", "POST"])
async def agriseva_models(request: Request):
    ...
    _log_request("/v1/agriseva/models", None, {"method": request.method})
    return JSONResponse(body)


@app.post("/v1/agriseva/chat/completions")
async def agriseva_chat_completions(request: Request):
    """
    OpenAI-compatible bridge to the AgriSeva-AI LangGraph agent.

    Requires LangGraph running (e.g. ``uv run langgraph dev`` in ``agriseva/ai``).
    ...
    context_headers = await _user_context_headers(request)
    _log_request(
        "/v1/agriseva/chat/completions",
        body,
        {"forwarded_headers": {**incoming, **context_headers}, "bridge": "langgraph"},
    )

    if body.get("stream", True):
        return EventSourceResponse(_stream_agriseva_response(body, request))
