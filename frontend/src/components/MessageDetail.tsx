...
const ContentAnswer = ({ text, question, isQuestionAllocatedToExpert, navigateToQuestionPage }: ContentAnswerProps) => {
    ...
    const handleAccept = () => {
        if (!question?._id) { toast.error("Question data is missing."); return; }
        if (question.source !== "AGRISEVA_AI" && question.source !== "WHATSAPP") { toast.error("Only AGRISEVA_AI or WHATSAPP answers can be approved."); return; }
    ...
    const handlePushToGDB = () => {
        ...
        if (question.source !== "AGRISEVA_AI" && question.source !== "WHATSAPP") {
            toast.error("Only AGRISEVA_AI or WHATSAPP answers can be approved."); return;
        }
    ...
    return (
        <>
            <div className="rounded-lg border-2 border-info/30 bg-card overflow-hidden">
                ...
                {question.status !== "dynamic" && approved === null && question && isAssignedModerator && (question.source == "AGRISEVA_AI" || question.source == "WHATSAPP") && question.status !== "closed" && !question.aiInitialAnswer && !isQuestionAllocatedToExpert && (
