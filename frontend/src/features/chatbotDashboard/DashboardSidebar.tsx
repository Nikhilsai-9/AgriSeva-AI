...
const SunIcon: React.FC = () => (
  <svg width={16} height={16} viewBox="0 0 16 16" fill="none">
    <path
      d="M8 2v2M8 12v2M2 8h2M12 8h2M4.5 4.5l1.4 1.4M10.1 10.1l1.4 1.4M4.5 11.5l1.4-1.4M10.1 5.9l1.4-1.4"
      stroke="currentColor"
      strokeWidth="1.2"
      strokeLinecap="round"
    />
...
export const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
  ...
  const handleNavClick = useCallback(
    (view: DashboardView, hasChildren?: boolean) => {
  ...
  const handleChildClick = useCallback(
    (childId: string, parentView: DashboardView) => {
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          {badge && (
            <span
              className={`text-[10px] font-medium px-1.5 py-px rounded-full text-white leading-none ${badgeVariant === "amber" ? "bg-[#BA7517]" : "bg-[#E24B4A]"}`}
            >
              {badge}
            </span>
          )}
      ...
      {collapsed && badge && (
        <span
          className={`absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full ${badgeVariant === "amber" ? "bg-[#BA7517]" : "bg-[#E24B4A]"}`}
        />
      )}
    </div>
  );
    ...
    return (
      <TooltipProvider delayDuration={150}>
        <Tooltip>
          ...
          <TooltipContent
            side="right"
            className="z-[100] font-medium shadow-md"
          >
            {label}
          </TooltipContent>
        </Tooltip>
...
const ChildNavItemRow: React.FC<ChildNavItemRowProps> = ({
  ...
  >
    <span
      className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? "bg-[#3AAA5A]" : "bg-(--border)"}`}
    />
    {label}
  </div>
);
