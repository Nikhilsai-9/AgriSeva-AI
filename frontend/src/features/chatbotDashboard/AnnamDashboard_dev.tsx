...
export function AnnamDashboard_dev({
  ...
  >(undefined);
  const [faqsDateRange, setFaqsDateRange] = useState<
    import("react-day-picker").DateRange | undefined
  >(undefined);
  ...
  const [customerNotificationsDateRange, setCustomerNotificationsDateRange] =
    useState<import("react-day-picker").DateRange | undefined>(undefined);
  ...
  const { data: queryCategories, isLoading: isLoadingQueryCategories } =
    useQueryCategories(source, filters.userType, shouldLoadQueryInsights);
  ...
  const { data: dailyQuestionTrendsData, isLoading: trendsLoading } =
    useDailyQuestionTrends(
      source,
  ...
  const kpiRow1WithOverlay =
    patchedKpiRow1
      ?.filter((card) =>
        (DYNAMIC_KPI_IDS as readonly string[]).includes(card.id),
      )
      .map((card) => ({ ...card, isDummy: false })) ?? [];
  const kpiRow2WithOverlay =
    data?.kpiRow2
      ?.filter((card) => card.id === "totalInstalls")
      .map((card) => ({ ...card, isDummy: card.id !== "totalInstalls" })) ?? [];
              ...
              {!mapView && source !== "acc" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6 items-stretch">
                  <ClosedQuestionsCard
                    ...
                    combinedCount={
                      questionStatusData?.closedVsTotalQuestions?.combined
                        ?.count
                    }
                    combinedAvgTime={
                      questionStatusData?.closedVsTotalQuestions?.combined
                  ...
                  <ClosedInLastTwoHoursCard
                    ...
                    totalClosed={
                      closed2hData?.closedInLastTwoHours?.totalClosedCount
                    }
                    ...
                    {source !== "whatsapp" && (
                      ...
                      >
                        <KnowledgeAwarenessCard
                          userMetricesData={
                            userMetricesData?.kccAndAgriAppUsage
                          }
                          hovered={hovered}
                  ...
                  >
                    {shouldLoadTrends ? (
                      <DailyQuestionTrendsChart
                        trends={(dailyQuestionTrendsData as any) ?? []}
