...
export const QAInterface = ({
  ...
  // Check if there are any timebound questions (AGRISEVA_AI or WHATSAPP) in the queue
  const hasTimeboundQuestions = useMemo(() => {
    return questions.some(
      (q) => q?.source === "AGRISEVA_AI" || q?.source === "WHATSAPP"
    );
  }, [questions]);
  ...
  useEffect(() => {
    ...
    const isCurrentTimeBound = q?.source === "AGRISEVA_AI" || q?.source === "WHATSAPP";
  ...
  useEffect(() => {
    ...
    // For AGRISEVA_AI: the answer comes from aiApprovedAnswer (answers collection)
    ...
    if (!newAnswer && !draft?.answer && selectedQuestionData.history?.length === 0) {
      ...
      if (selectedQuestionData.source === 'AGRISEVA_AI') {
    ...
    // For AGRISEVA_AI questions, pre-fill sources from moderator-approved sources stored on the question
    if (
      selectedQuestionData.source === 'AGRISEVA_AI' &&
