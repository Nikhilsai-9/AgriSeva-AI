# Initial Setup — AgriSeva-AI AI (`ai/`)

Commands below assume a Unix-like shell (Linux or macOS) and that you work from the `ai` directory:

```bash
cd /path/to/agriseva/ai
```

Replace `/path/to/agriseva` with your clone path (for example `/app/agriseva`).
...
```json
{
  "dependencies": ["."],
  "graphs": {
    "agriseva": "./agriseva/agents/agriseva.py:graph"
  },
  "env": ".env"
}
```
...
- **`graphs`** — expose the compiled `graph` object from `agriseva/agents/agriseva.py`
...
```bash
uv run python -m agriseva.app
```

(Default bind is defined in `agriseva/app.py`.)
