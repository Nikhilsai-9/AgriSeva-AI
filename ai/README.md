# AgriSeva-AI AI Agent

## Overview

AgriSeva-AI is an AI assistant for Indian farmers that answers queries about crop diseases, pests, fertilizers, soil health, market prices, weather, and government schemes — in any Indian language.
...
| Planner orchestrator graph | ✅ Done | `USE_PLANNER_GRAPH=true` (default): planner → execute_plan → synthesize; see `agriseva/agents/PLANNER_PRODUCT_DECISIONS.md` |
...
```bash
python -m pytest agriseva/agents/tests/test_planner.py -v
python -m pytest agriseva/agents/tests/ -v
```

Routing tests in `test_routing.py` call the legacy `agriseva_node` directly and require Anthropic API + MCP. Planner unit tests do not.
