...
from agriseva.agents.config import CLAUDE_MODEL, PLANNER_MODEL, get_minimax_chat_model, resolve_thread_id, resolve_user_id
from agriseva.agents.thread_logging import (
    begin_conversation_turn,
    end_conversation_turn,
)
from agriseva.agents.crop_chemical_resolver import format_planner_crop_hints
from agriseva.agents.thread_trace import trace_event
from agriseva.agents.crop_requirement import (
    CropRequirementDecision,
    is_crop_specific_question,
)
from agriseva.agents.domains import (
...
from agriseva.agents.language import _llm_detect_language, detect_script_language, resolve_planner_language_pair
from agriseva.agents.translation_catalog import (
    OFFICIAL_LANGUAGES,
    get_catalog,
    get_crop_follow_up,
    get_state_follow_up,
...
from agriseva.agents.plan_executor import ENABLE_CHEMICAL_CHECKER
from agriseva.agents.llm_trace import trace_llm_request, trace_llm_response
from agriseva.agents.resolution_trace import trace_resolution, trace_thread_location
from agriseva.agents.planner_rules import (
...
from agriseva.agents.prompts import PLANNER_SYSTEM_PROMPT
from agriseva.agents.state import AgriSevaState, PlannerEntities, PlannerPlan
from agriseva.agents.user_location import (
    load_user_location,
...
class PlannerOutput(BaseModel):
    domains: list[str] = Field(
        ...
        description=(
            "1-3 values from ALLOWED_DOMAINS in agriseva.agents.domains "
            "(most relevant first)."
        ),
        min_length=1,
        max_length=3,
...
def clarify_node(state: AgriSevaState) -> dict:
...
def route_after_planner(state: AgriSevaState) -> str:
    plan = state.get("plan") or {}
    if plan.get("is_follow_up"):
        return "follow_up"
...
def route_after_ensure_location(state: AgriSevaState) -> str:
    plan = state.get("plan") or {}
