...
_AGRISEVA_AI_MARKDOWN_SOURCES = frozenset({"agriseva", "agriseva_webapp"})


def is_agriseva_markdown_source_client(question_source: str | None) -> bool:
    """True when GDB sources should render as markdown links for the client."""
    if not question_source:
        return False
    return question_source.strip().lower() in _AGRISEVA_AI_MARKDOWN_SOURCES
...
def _format_source_line(
    ...
    *,
    ...
    """Format one source line — markdown for AgriSeva-AI clients, plain text otherwise."""
    source_name = details.get("source_name")
    source_link = (details.get("source_link") or "").strip()

    if is_agriseva_markdown_source_client(question_source):
        if source_link:
...
def collect_all_sources(
    ...
    *,
    ...
    Output format:
        ...
        📚 Sources:
        [source](https://link)   — AgriSeva-AI clients
