...
from agriseva.agents.synthesizer import _format_tool_results_for_synthesizer
from agriseva.agents.plan_executor import execute_plan_node
...
async def test_build_tool_calls_includes_reviewer_and_weather():
    ...
    assert reviewer["args"]["source"] == "AGRISEVA_AI"
...
@pytest.mark.asyncio
async def test_reviewer_upload_includes_identity_fields_for_agriseva():
    plan = {
        "weather": False,
        "mandi": False,
        "soil": False,
...
@pytest.mark.asyncio
async def test_reviewer_upload_omits_identity_fields_for_non_agriseva():
    plan = {
        "weather": False,
        "mandi": False,
        "soil": False,
...
def test_merge_plan_replaces_stale_entities_on_new_planner_turn():
    from agriseva.agents.state import merge_plan
