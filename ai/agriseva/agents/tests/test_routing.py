"""
Live routing tests for the legacy agriseva_node (bind_tools + WHATSAPP_SYSTEM_PROMPT).
...
from agriseva.agents.agriseva import agriseva_node, use_planner_graph
from agriseva.agents.state import AgriSevaState

pytestmark = pytest.mark.skipif(
    use_planner_graph(),
    reason="Legacy agriseva_node tests; set USE_PLANNER_GRAPH=false to run",
)
...
async def test_weather_intent_routing(base_config):
    ...
    result = await agriseva_node(state, base_config)
...
async def test_greeting_uploads_to_reviewer(base_config):
    ...
    """
    state: AgriSevaState = {
        "messages": [HumanMessage(content="Namaste AgriSeva-AI!")],
        "location": None
    }
