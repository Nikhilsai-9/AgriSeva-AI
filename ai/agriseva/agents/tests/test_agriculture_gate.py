...
from agriseva.agents.agriseva import graph, use_planner_graph
from agriseva.agents.plan_executor import build_reviewer_upload_calls, build_tool_calls_from_plan
from agriseva.agents.planner import route_after_ensure_location
...
async def test_upload_reviewer_only_node_ignores_reviewer_answer_text():
    from agriseva.agents.plan_executor import upload_reviewer_only_node
    ...
    with patch(
        "agriseva.agents.plan_executor.get_main_tool_node",
        AsyncMock(return_value=mock_tool_node),
    ), patch(
        "agriseva.agents.plan_executor.get_location_tool",
        AsyncMock(return_value=mock_location),
    ), patch(
        "agriseva.agents.plan_executor.get_reviewer_tool",
        AsyncMock(return_value=mock_reviewer),
    ):
        result = await upload_reviewer_only_node(state, {"configurable": {"question_source": "AGRISEVA_AI"}})
