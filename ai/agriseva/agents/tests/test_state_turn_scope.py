...
from agriseva.agents.plan_executor import build_tool_calls_from_plan
from agriseva.agents.planner import _resolve_state_deterministic
from agriseva.agents.planner_rules import apply_planner_completeness_rules
...
async def test_state_does_not_leak_on_new_question_with_gps():
    from agriseva.agents.planner import planner_node
...
async def test_location_clarification_preserves_catalog_language_pair(
    ...
    from agriseva.agents.planner import PlannerOutput, planner_node
    from langchain_core.runnables import RunnableConfig
    ...
    with (
        patch("agriseva.agents.planner.get_minimax_chat_model", return_value=model),
        patch("agriseva.agents.planner._llm_detect_language", detected_language),
    ):
