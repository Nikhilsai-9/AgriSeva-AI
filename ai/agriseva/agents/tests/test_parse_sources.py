...
from agriseva.agents.answer_footers import (
    FOOTER_SEPARATOR,
    collect_all_sources,
    finalize_synthesis_answer,
    is_agriseva_markdown_source_client,
)
from agriseva.tools.golden.golden_core import parse_sources as _parse_sources
...
def test_is_agriseva_markdown_source_client():
    assert is_agriseva_markdown_source_client("AGRISEVA_AI") is True
    assert is_agriseva_markdown_source_client("AGRISEVA_AI_WEBAPP") is True
    assert is_agriseva_markdown_source_client("agriseva_webapp") is True
    assert is_agriseva_markdown_source_client("WHATSAPP") is False
    assert is_agriseva_markdown_source_client(None) is False
...
def test_collect_all_sources_link_only_footer_agriseva_markdown():
    gdb_data = {
        "is_exact": True,
        "exact_match": {
            "question": "Q",
    ...
    block = collect_all_sources(gdb_data, question_source="AGRISEVA_AI")
    assert "[source](https://workdrive.zohoexternal.in/file/xyz)" in block
    assert "Dheeraj Sharma" in block
    assert "🔗" not in block


def test_collect_all_sources_named_source_agriseva_markdown():
    gdb_data = {
        "is_exact": True,
        "exact_match": {
            "question": "Q",
    ...
    block = collect_all_sources(gdb_data, question_source="AGRISEVA_AI_WEBAPP")
    assert "[PAU Handbook](https://example.edu/doc)" in block


def test_collect_all_sources_multiple_unnamed_agriseva_markdown():
    gdb_data = {
        "is_exact": True,
        "exact_match": {
            "question": "Q",
