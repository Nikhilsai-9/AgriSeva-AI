...
from agriseva.agents import thread_logging as tl
...
def test_thread_file_handler_routes_by_context(tmp_path: Path):
    ...
    logger = logging.getLogger("agriseva.agents.tests.thread_logging")
...
def test_thread_file_handler_closes_each_log_stream(tmp_path: Path, monkeypatch):
    ...
    handler = tl.ThreadFileLogHandler(tmp_path)
    handler.addFilter(tl.ThreadLogFilter())
    logger = logging.getLogger("agriseva.agents.tests.thread_logging_close")
    logger.addHandler(handler)
    logger.setLevel(logging.INFO)
...
def test_turn_buffer_captures_handler_logs(tmp_path: Path, monkeypatch):
    ...
    handler = tl.ThreadFileLogHandler(tmp_path)
    handler.addFilter(tl.ThreadLogFilter())
    logger = logging.getLogger("agriseva.agents.tests.turn_buffer")
    logger.addHandler(handler)
    logger.setLevel(logging.INFO)
...
def test_handler_emit_writes_file_only_not_mongo_per_line(tmp_path: Path, monkeypatch):
    ...
    handler = tl.ThreadFileLogHandler(tmp_path)
    handler.addFilter(tl.ThreadLogFilter())
    logger = logging.getLogger("agriseva.agents.tests.thread_logging_mongo")
    logger.addHandler(handler)
    logger.setLevel(logging.INFO)
