...
from agriseva.tools.golden.gemma_classifier import (
...
from agriseva.tools.golden.golden_core import QuestionAnswerPair
...
async def test_select_tie_breaker_when_two_same_intent(monkeypatch):
    ...
    monkeypatch.setattr(
        "agriseva.tools.golden.gemma_classifier.tie_breaker",
        mock_tie_breaker,
    )
    pairs = [_pair("a", 0.7), _pair("b", 0.95)]
    classifications = [
...
async def test_gdb_search_same_question_bypass(monkeypatch):
    from agriseva.tools.golden import golden_search
