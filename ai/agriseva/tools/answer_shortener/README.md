# AgriSeva-AI Answer Shortener API

Standalone FastAPI service that uses Claude Sonnet only to rank immutable
segments from an existing AgriSeva-AI answer. Python selects and copies the
...
```json
{
  ...
  "answer": "The existing AgriSeva-AI answer body...\n\n👤 Answered by: Expert name\n📚 Sources: KVK advisory\n⚠️ Warning text",
...
From `C:\agriseva\ai`, create the project-level environment if it does not
already exist:

```powershell
py -3.11 -m venv .venv
& .\.venv\Scripts\python.exe -m pip install -r .\agriseva\tools\answer_shortener\requirements-dev.txt
```

For a non-Docker run, an existing `C:\agriseva\ai\.env` can be used instead;
...
```powershell
Remove-Item Env:PYTHONPATH -ErrorAction SilentlyContinue; & 'C:\agriseva\ai\.venv\Scripts\python.exe' -m uvicorn agriseva.tools.answer_shortener.api:app --app-dir 'C:\agriseva\ai' --host 127.0.0.1 --port 8007
```
...
```powershell
Set-Location C:\agriseva\ai\agriseva\tools\answer_shortener
...
```powershell
Remove-Item Env:PYTHONPATH -ErrorAction SilentlyContinue; & 'C:\agriseva\ai\.venv\Scripts\python.exe' -m pytest agriseva/tools/answer_shortener/tests -q
```
